...
<div class="container mt-4">
    ...
    <form action="{{ route('nosql.use_case.appointment.store') }}" method="POST">
                ...
                @foreach ($repairServices as $service)
                    <div class="col-md-4 mb-4">
                        <div class="card p-3 shadow-sm">
                            ...
                            <input type="checkbox" name="service_ids[]" value="{{ $service['_id'] }}" class="form-check-input" data-price="{{ $service['price'] }}" onchange="calculateTotalPrice()">
        ...
        <div id="step-2" style="display: none;">
            ...
            @if (empty($serviceMethods))
                <p class="text-danger">No service methods available. Please check your MongoDB collection.</p>
            @endif
            <div class="row">
                @foreach ($serviceMethods as $method)
                    <div class="col-md-4 mb-4">
                        <div class="card p-3 shadow-sm">
                            ...
                            <input type="radio" name="method_id" value="{{ $method['_id'] }}" data-cost="{{ $method['cost'] }}" class="form-check-input" onchange="calculateTotalPrice()">
                            <label class="form-check-label">Select</label>
                        </div>
                    </div>
                @endforeach
        ...
        <div id="step-4" style="display: none;">
            ...
            <div class="mb-3">
                ...
                <input type="text" id="street_name" name="street_name" class="form-control" value="{{ $customerDetails['address']['street_name'] ?? '' }}">
            </div>

            <div class="mb-3">
                <label class="form-label">House Number</label>
                <input type="text" id="house_number" name="house_number" class="form-control" value="{{ $customerDetails['address']['house_number'] ?? '' }}">
            </div>

            <div class="mb-3">
                <label class="form-label">City</label>
                <input type="text" id="city" name="city" class="form-control" value="{{ $customerDetails['address']['city'] ?? '' }}">
            </div>

            <div class="mb-3">
                <label class="form-label">Postal Code</label>
                <input type="text" id="postal_code" name="postal_code" class="form-control" value="{{ $customerDetails['address']['postal_code'] ?? '' }}">
            </div>
...
<script>
    ...
    function updateTimeSlots() {
        ...
        let serviceDuration = selectedMethod ? parseInt(selectedMethod.getAttribute("data-duration")) : 60;
        ...
        workingHours.forEach(slot => {
            ...
            if (selectedDate.toDateString() === today.toDateString() && slotHour <= currentHour) return;

            if ((slotHour + serviceDuration / 60) > 18) return;
